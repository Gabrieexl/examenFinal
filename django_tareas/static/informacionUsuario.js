...
function finalizarTarea(idFinalizar) {
    let taskId = idFinalizar.split("finalizar")[1];
    let estadoCell = document.getElementById("estado" + taskId);
    if (estadoCell) {
        estadoCell.innerHTML = "Finalizado";
    }

    fetch(`/actualizar_estado_tarea/${taskId}/?estado=Finalizado`, {
        method: 'POST',
        headers: {
            "X-Requested-With":"XMLHttpRequest",
            'X-CSRFToken': getCookie('csrftoken')
